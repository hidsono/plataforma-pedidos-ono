...
import { PackageOpen, PlusCircle, Search, X, Store, MapPin, Clock } from 'lucide-react';
...
export default function Home() {
  ...
  const [storeStatus, setStoreStatus] = useState<{ isOnlineOpen: boolean; closedMessage: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Atualiza status de horário de funcionamento
    const updateBizStatus = () => setBusinessStatus(getBusinessStatus());
    updateBizStatus();
    const bizInterval = setInterval(updateBizStatus, 60000);
  ...
  // A plataforma fica fechada se: admin fechou manualmente OU fora do horário de funcionamento
  const isManualClosed = storeStatus !== null && !storeStatus.isOnlineOpen;
  const isOutsideHours = businessStatus !== null && !businessStatus.isOpen;
  const isStoreClosed = isManualClosed || isOutsideHours;
      ...
      <div className="container animate-fade-in" style={{ paddingBottom: '60px' }}>
        ...
        {isStoreClosed && (
          <div className="animate-fade-in" style={{
            ...
            padding: '24px',
            borderRadius: '16px',
            background: 'linear-gradient(135deg, rgba(220,53,69,0.08) 0%, rgba(255,152,0,0.08) 100%)',
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
              <div style={{
                ...
                background: 'linear-gradient(135deg, rgba(220,53,69,0.15), rgba(255,152,0,0.15))',
                display: 'flex', alignItems: 'center', justifyContent: 'center'
              }}>
                {isOutsideHours
                  ? <Clock size={26} style={{ color: '#dc3545' }} />
                  : <Store size={26} style={{ color: '#dc3545' }} />
                }
              </div>
              <div style={{ flex: 1 }}>
                <h2 style={{ fontSize: '1.15rem', marginBottom: '6px', color: '#dc3545', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {isOutsideHours
                    ? (businessStatus?.message || 'Estamos Fechados')
                    : 'Pedidos Online Temporariamente Fechados'
                  }
                </h2>
                <p style={{ color: 'var(--secondary-text)', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '12px' }}>
                  {isOutsideHours
                    ? `Nosso horário de pedidos encerrou. ${businessStatus?.nextOpening ? `Voltaremos: ${businessStatus.nextOpening}.` : ''}`
                    : storeStatus?.closedMessage
                  }
                </p>

                {/* Mostra badge "loja física aberta" apenas se admin fechou mas está dentro do horário */}
                {isManualClosed && !isOutsideHours && (
                  <div style={{
                    display: 'inline-flex', alignItems: 'center', gap: '6px',
                    padding: '6px 14px', borderRadius: '99px',
                    backgroundColor: 'rgba(26, 131, 85, 0.1)',
                    color: 'var(--success)',
                    fontSize: '0.85rem', fontWeight: 600,
                    border: '1px solid rgba(26, 131, 85, 0.2)'
                  }}>
                    <MapPin size={14} />
                    Loja física aberta para atendimento
                  </div>
                )}

                {/* Mostra próximo horário se fora de horário */}
                {isOutsideHours && businessStatus?.nextOpening && (
                  <div style={{
                    display: 'inline-flex', alignItems: 'center', gap: '6px',
                    padding: '6px 14px', borderRadius: '99px',
                    backgroundColor: 'rgba(30, 109, 155, 0.1)',
                    color: 'var(--primary)',
                    fontSize: '0.85rem', fontWeight: 600,
                    border: '1px solid rgba(30, 109, 155, 0.2)'
                  }}>
                    <Clock size={14} />
                    Reabre: {businessStatus.nextOpening}
                  </div>
                )}
              </div>
            </div>
        ...
        <div style={{ marginTop: isStoreClosed ? '10px' : '40px', marginBottom: '20px' }}>
          <h1 style={{ fontSize: '2rem', marginBottom: '8px' }}>Nosso Catálogo</h1>
          <p style={{ color: 'var(--secondary-text)' }}>
            {isStoreClosed
              ? (isOutsideHours
                ? 'Confira nossos produtos. Os pedidos voltam no próximo horário de funcionamento.'
                : 'Confira nossos produtos abaixo. Os pedidos online estão temporariamente fechados.')
              : 'Escolha seus produtos e faça o pedido de forma rápida e segura.'
            }
          </p>
              ...
              {productsToShow.map(product => (
                <div className="product-card" key={product.id}>
                  ...
                  <div className="product-content">
                    ...
                    <button
                      className={`btn ${isStoreClosed ? 'btn-secondary' : 'btn-primary'}`}
                      style={{
                        width: '100%', marginTop: 'auto',
                        ...(isStoreClosed ? { opacity: 0.5, cursor: 'not-allowed' } : {})
                      }}
                      onClick={() => handleAddToCartClick(product)}
                      disabled={isStoreClosed}
                    >
                      <PlusCircle size={18} />
                      {isStoreClosed ? 'Indisponível' : 'Adicionar'}
                    </button>
                  </div>
                </div>
              ))}
